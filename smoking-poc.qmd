...
smok_ratios_gender <- smok_age_bands |>
    dplyr::select(age_band, Gender, tot_smokers) |>
    group_by(age_band, Gender) |>
    reframe(nat_smokers = sum(tot_smokers)) |>
    group_by(age_band) |>
    reframe(att_ratio = nat_smokers[2] / nat_smokers[1]) |>
    ungroup() |> 
    mutate(mean_ratio = mean(att_ratio, na.rm = TRUE))


```

The relative attendance rate for males is `r smok_ratios_gender$mean_ration[1]` times that of females.
...
pop_age |>
    ...
    phe_rate(x = tot_smokers, n = sumpops) |>
...
The relative attendance rate for males is
